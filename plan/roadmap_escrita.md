...
> **Premissa da tabela (25/jul):** **1 seção pesada/sábado**; itens leves (cap. 1, cap. 6, pré-textuais, acabamento, §5.1/§5.3) saem em **noites**. **2 sábados: 25/jul cap. 5 · 01/ago cap. 3.** **Cap. 2 cortado** (não entra). Núcleo ao Icaro assim que fecha (cap. 4 já pronto). **Sem sábado de revisão** — revisão incremental nas noites 03–06/ago. **8–26/ago = pós-entrega.**
...
| **W9** | 03–07 ago | *Noites 03–06:* fechar **cap. 1** + **cap. 6 + pré-textuais** (sem `\lipsum`) **e revisar incremental** (consistência, `\ref`/`\label`, `.bib`, siglas, compilação). **Congelar conteúdo ~qui 06/ago.** *Sex 07 (manhã):* **passe final** — PDF limpo, checklist de "pronto", **entregar à banca**. | **PDF final entregue (sex 7/ago)** |
...
- **~qui 06/ago (W9):** **congelamento de conteúdo** — todos os capítulos + cap. 6 + pré-textuais (sem `\lipsum`). *(Cap. 2 e cap. 7 não entram — cortados.)* A partir daqui só revisão.
...
2. **Acabamento não-bloqueante** (nomes entre tabelas, `\cite` de firmware) → só se sobrar; não pode atrasar a compilação limpa.

**Nunca cortar** (define a qualificação sob enquadramento B): pergunta de pesquisa clara, protocolo do experimento (4.6), **o caso setpoint escalar (único, intocável)**, **o núcleo da fundamentação (3.3 modelo de memória/data race + 3.4 ownership/`Send`-`Sync`/`unsafe`)**, o eixo "fronteira do que Rust não garante" (honestidade), o **cronograma da dissertação (cap. 6)** — o fecho do documento agora que a conclusão (cap. 7) foi removida —, e **um passe de revisão final** (não entregar cru). A entrega de 7/ago **não se move**.
