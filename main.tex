\documentclass[tcc]{ic}

\hypersetup{
    colorlinks = {true},
    linktocpage = {false},
    plainpages = {false},
    linkcolor = {Blue},
    citecolor = {Blue},
    urlcolor = {Red},
    unicode = {true},
    pdftitle = {Título do trabalho}, 
    pdfauthor = {Fulano da Silva Alves},
    pdfsubject = {Proposta de Qualificação de Mestrado},
    pdfkeywords = {processamento gráfico, imagens médicas, visão computacional, aprendizagem profunda, aumento de dados, graphics processing, medical images, computer vision, deep learning, data augmentation}
}

\usepackage{algorithm}
\usepackage{algpseudocode}
\usepackage{enumerate}
\usepackage{fancyhdr}
\usepackage{acronym}
\usepackage{natbib}
\usepackage{lipsum}
\usepackage{amsmath}
% added-by: Claude (claude-opus-4-8), 2026-07-28 -- suporte a tabela CSV do apendice (res/pids.csv), autorizado por Matheus
\usepackage{longtable}   % tabela que quebra em varias paginas
\usepackage{pdflscape}   % pagina em landscape para a tabela larga
\usepackage{csvsimple}   % le res/pids.csv em tempo de compilacao

\makeatletter
\renewcommand{\ALG@name}{Algoritmo}
\renewcommand{\listalgorithmname}{List of \ALG@name s}
\makeatother

\newcolumntype{Y}{>{\centering\arraybackslash}X}

\begin{document}

    % Inclui o preâmbulo do documento (Informações da capa e contracapa)
    \include{pre-textuais/preambulo}

    \selectlanguage{portuguese}
    
    % Capa, contracapa e avaliadores
    \capa
    
    % Agradecimentos
    % \include{pre-textuais/agradecimentos}
    
    % Resumo e Abstract
    \include{pre-textuais/resumo}
    \include{pre-textuais/abstract}
    \include{pre-textuais/LISTA_DE_TABELAS}
    \include{pre-textuais/LISTA_DE_FIGURAS}
    \include{pre-textuais/LISTA_DE_SIGLAS}
 
    
    % Sumário
    \tableofcontents
    \thispagestyle{empty}
    
    % Início dos capítulos
    \inicio
    
    \include{capitulos/introducao}
    \include{capitulos/fundamentacao}
     \include{capitulos/Metodologia_Proposta}
     \include{capitulos/Resultados_Parciais}
     \include{capitulos/Cronograma_de_Execucao}
\appendix
\include{pos-textuais/apendiceA}

\cleardoublepage
\lhead{Bibliografia}
%\bibliographystyle{estilo_}
\bibliography{referencias} % arquivo fonte com a bibilografia
\addcontentsline{toc}{chapter}{Bibliografia}


    
    % Pós Textuais
    % \nocite{*}
    % \include{pos-textuais/referencias}

\end{document}
