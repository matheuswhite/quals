...
- **Code and config files** (`src/**`, `Makefile`s, `.gitignore`, and similar build/config) — **attributed to Matheus, not Claude** (decision 2026-07-23): he directs the design and can defend every line; Claude only transcribes. So these files carry **no authorship header/comment**, and their commits carry **no `Co-Authored-By: Claude` trailer** (see "Commit message conventions"). This is the sole exception to the rule above. The `.bib` / triagem `% added-by: Claude` audit trail (Rule 7) is **preserved** — it exists for banca honesty and is untouched by this carve-out.
...
- **Code/config carve-out (2026-07-23, see Rule 5):** commits that touch only code/config files (`src/**`, `Makefile`s, `.gitignore`) are attributed to Matheus — use the normal prefixes (`feat:`/`fix:`/`refactor:`/`docs:`/`chore:`), **not** `ai:`, and carry **no `Co-Authored-By: Claude` trailer**. `ai:` + the trailer stay for changes still attributed to Claude (e.g. `plan/*.md`, `.bib` entries).
